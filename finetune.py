...
def run_stage(args, stage, fine_tune_model, train_loader, dev_loader, test_loader):
    ...
    val_check_interval = args.stage_2_check_interval
    if stage == 1:
        val_check_interval = args.stage_1_check_interval
    ...
    trainer.fit(fine_tune_model, train_dataloaders=train_loader, val_dataloaders=dev_loader)
    best_ckpt = checkpoint_callback.best_model_path
    if stage == 1:
        print(f"Stage 1 over, epoch: {fine_tune_model.current_epoch }, best ckpt: {best_ckpt}")
    else:
        print(f"Stage 2 over, epoch: {fine_tune_model.current_epoch }, best ckpt: {best_ckpt}")
...
def main(args):
    ...
    state_dict = torch.load(args.pretrain_ckpt, map_location="cpu")


    if "state_dict" in state_dict:
        state_dict = state_dict["state_dict"]
    fine_tune_model.model.resize_token_embeddings(len(tokenizer))
    fine_tune_model.load_state_dict(state_dict, strict = False)
    ...
    if args.best_ckpt != None:
        ...
        test_loader = DataLoader(test_data, 
                                ...
                                num_workers=args.dataloader_num_workers)
        best_ckpt = run_stage(args, 1, fine_tune_model, train_loader, dev_loader, test_loader)

    fine_tune_model.load_state_dict(torch.load(best_ckpt, map_location="cpu")["state_dict"], strict = True)
    fine_tune_model.update_item_emb = False
    fine_tune_model.model.use_mlm = False
    
    finetune_data_collator = TrainCollator(tokenizer, tokenized_items, mlm_probability = args.mlm_probability, use_mlm=False, use_img = config.use_img)
    eval_data_collator = EvalCollator(tokenizer, tokenized_items, use_img = config.use_img)
    ...
    run_stage(args, 2, fine_tune_model, train_loader, dev_loader, test_loader)
